...
struct sdmc_gpio_devs {
	...
	struct sdmc_gpio_dev standby_sync;
};
...
static void sdmc_gpio_init(void)
{
	...
	if (NOUSED_GPIO !=gdev->standby_sync.pin) {
		nRet = sdmc_req_gpio(gdev->standby_sync.pin, GPIO_INPUT, 1, NULL);
		if (0 != nRet)
			printk("%s Failed, line = %d!\n", __FUNCTION__, __LINE__);
	}
...
static ssize_t store_standby_sync(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (r != 1)
		return -EINVAL;
    if (1 == dbg)
		sdmc_set_gpio(gdev->standby_sync.pin, 1);
	else
		sdmc_set_gpio(gdev->standby_sync.pin, 0);
...
static int sdmc_gpio_dt_parse(struct platform_device *pdev)
{
	...
	desc = of_get_named_gpiod_flags(node, "net_led", 0, &flags);
	gdev->net_led.active = flags & OF_GPIO_ACTIVE_LOW;
	gdev->net_led.pin = desc_to_gpio(desc);
	printk("###$$$ sdmc_gpio->net_led.pin = %d flags = %d\n", gdev->net_led.pin, flags);
	...
	gdev->s2_ant_pwr.active = flags & OF_GPIO_ACTIVE_LOW;
	if (gdev->s2_ant_pwr.active == 1)
		gdev->s2_ant_pwr.pin = -1;
	else
		gdev->s2_ant_pwr.pin = desc_to_gpio(desc);
	printk("###$$$ sdmc_gpio->s2_ant_pwr.pin = %d flags = %d\n", gdev->s2_ant_pwr.pin, flags);
	...
	desc = of_get_named_gpiod_flags(node, "standby_sync", 0, &flags);
	gdev->standby_sync.active = flags & OF_GPIO_ACTIVE_LOW;
	if (gdev->standby_sync.active == 1)
		gdev->standby_sync.pin = -1;
	else
		gdev->standby_sync.pin = desc_to_gpio(desc);
	printk("###$$$ sdmc_gpio->standby_sync.pin = %d flags = %d\n", gdev->standby_sync.pin, flags);
