...
struct sdmc_gpio_dev {
    struct sdmc_gpio sdmc_led_blink;
}
...
static int sdmc_gpio_dt_parse(struct platform_device *pdev)
{
    ...
    gdev->sdmc_led_blink.active = flags & OF_GPIO_ACTIVE_LOW;
    gdev->sdmc_led_blink.pin = desc_to_gpio(desc);
...
#endif
    kfree(gdev);
    printk("sdmc led blink module removed ok\n");
